async function setupFyp(){
    ...
    if (allPosts.success && userData.found){
    var uPosts = allPosts.userPosts;
    var validInterests = userData.fUser[0].interests;

    for(i=0; i<uPosts.length; i++){

        if(validInterests.includes(uPosts[i].eventType)){
            let card = 
            '<div class="my-5 ml-5">'+
        ...
        '<div class="my-auto ml-5 py-5">'+
            '<div class="card dropShadow" style="width: 30rem">'+
                '<div class="card-body">'+
                    ...
                    `<p class="card-text">Molimo Vas da se registrujete, ili vratite na sajt kasnije, da biste videli sadržaj prilagođen vašim interesovanjima.</p>`+
